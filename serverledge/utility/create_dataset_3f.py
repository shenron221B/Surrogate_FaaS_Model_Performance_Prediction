...
def main():
    ...
    for row in rows:
        ...
        if jtl_files:
            ...
            diffs = []
            for func_id in range(FUNC_OFFSET, FUNC_OFFSET + FUNCTIONS):
                overhead = calculate_net_overhead(os.path.join(row_path, f"http_responses_func{func_id}.txt"), df, f"func_{func_id}")
    ...
    config = {
        ...
        "serv_time_cvs": cvs,
        "init_times": np.nanmean(np.array(mean_inits), axis=0).tolist(), "mem_demands": [1024] * FUNCTIONS,
