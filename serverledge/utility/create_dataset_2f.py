...
def compute_utility_mean(df, func_label, mean_dur):
    ...
    total_invocation = len(
        df[df["label"] == f"Invoke_{func_label}"])
    if mean_dur > 0:
...
def compute_response_time_median(df, func_label):
    df_func = df[df["label"] == f"Invoke_{func_label}"]
    df_func = df_func[df_func["responseCode"].astype(str) == "200"]
    return np.median(df_func["Latency"] / 1000) if not df_func.empty else 0
...
def main():
    ...
    for idx_row, row in enumerate(rows):
        ...
        for f in range(FUNC_OFFSET, FUNC_OFFSET + FUNCTIONS):
            ...
            if not df.empty:
                rt_client = compute_response_time_median(df, func_label)  # Usiamo la mediana
